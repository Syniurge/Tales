...
void CodeGenTypes::addRecordTypeName(const RecordDecl *RD,
  ...
  // NOTE: For Tales! make_absolute may be overkill, I realized I forgot it in PseudoHeaderGen yet it's been working.
  const std::string &InputConstraint = CGM.getCodeGenOpts().InputConstraint;
  PresumedLoc PLoc = Context.getSourceManager().getPresumedLoc(RD->getLocation());
  if (!PLoc.isInvalid()) {
    SmallString<256> PLocFilename(PLoc.getFilename());
    llvm::sys::fs::make_absolute(PLocFilename);
//    if (RD->isThisDeclarationADefinition())
    if (InputConstraint.compare(PLocFilename.substr(0, InputConstraint.size())) == 0)
      TheModule.addNamedStructType(OS.str(), Ty);
  }
}
