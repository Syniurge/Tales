...
// Tales FIXME: Temporary definitions, should go in Decl.h once we're done
bool NamedDecl::isWhitelisted() const { return cast<NamedDecl>(getCanonicalDecl())->Whitelisted; }

void NamedDecl::Whitelist() {
  cast<NamedDecl>(getCanonicalDecl())->Whitelisted = true;
  llvm::errs() << "Whitelisting " << getName() << " of kind " << getDeclKindName() << "\n";
}
  ...
  if (getLangOpts().CPlusPlus) {
    ...
    if (OldMethod && NewMethod) {
      ...
      if (!isFriend && NewMethod->getLexicalDeclContext()->isRecord() &&
          !IsClassScopeExplicitSpecialization && !IsPHParsing) {
...
void Sema::MergeVarDecl(VarDecl *New, LookupResult &Previous,
  ...
  if (Old->isStaticDataMember() && !New->isOutOfLine() && !IsPHParsing) {
...
Sema::ActOnTypedefDeclarator(Scope* S, Declarator& D, DeclContext* DC,
  ...
  if (IsPHParsing) {
    ...
    if (ND->isInvalidDecl()) {
      Diag(D.getIdentifierLoc(), diag::err_whitelisted_no_matching_decl_found) << D.getIdentifier();
    } else {
      Previous.getFoundDecl()->Whitelist();
      Context.Deallocate(NewTD);
    }
    return 0;
  }
...
Sema::ActOnVariableDeclarator(Scope *S, Declarator &D, DeclContext *DC,
  ...
  if (IsPHParsing) {
    // At this point we are about 400% sure that the declaration
    // in Previous is the one we need to whitelist.
    assert(!Previous.empty());
    Previous.getFoundDecl()->Whitelist();
    Context.Deallocate(NewVD);
    return 0;
  }
...
Sema::ActOnFunctionDeclarator(Scope *S, Declarator &D, DeclContext *DC,
  ...
  if (IsPHParsing) {
    Previous.getFoundDecl()->Whitelist();
    NewFD->DeclContext::~DeclContext();
    Context.Deallocate(NewFD);
    return 0;
  }
  ...
  if (IsPH2BC() && D.getFunctionDefinitionKind() == FDK_Declaration)
    LateDecls.push_back(NewFD);
...
Sema::FinalizeDeclaratorGroup(Scope *S, const DeclSpec &DS,
  ...
  if (DeclSpec::isDeclRep(DS.getTypeSpecType()) && !IsPHParsing)
...
Decl *Sema::ActOnTag(Scope *S, unsigned TagSpec, TagUseKind TUK,
    ...
    if (TemplateParameterList *TemplateParams
      ...
      if (TemplateParams->size() > 0) {
        ...
        DeclResult Result = CheckClassTemplate(S, TagSpec, TUK, KWLoc,
                                               ...
                                               TemplateParameterLists.data());
        if (!Result.isInvalid() && IsPHParsing)
          cast<NamedDecl>(Result.get())->Whitelist();
        return Result.get();
      } else {
  ...
  if (TUK == TUK_Friend || TUK == TUK_Reference || IsPHParsing)
  ...
  if (Name && SS.isNotEmpty()) {
    ...
    // Tales - This is an anonymous struct or union def FIXME TODO FIXME
    // We cannot identify which one it is straight away but its field names have to
    // be distinct from the rest of the parent scope, unlike named structs, so we
  ...
  if (!Previous.empty()) {
      ...
      if (TUK == TUK_Reference || TUK == TUK_Friend ||
        ...
        if (Kind == TTK_Enum && PrevTagDecl->getTagKind() == TTK_Enum) {
          ...
          if (CheckEnumRedeclaration(NameLoc.isValid() ? NameLoc : KWLoc,
            ...
            if (IsPHParsing)
              PrevEnum->getDefinition()->Whitelist();
        ...
        if (!Invalid) {
          ...
          if (TUK == TUK_Definition) {
            if (IsPHParsing) {
              if (!PrevTagDecl->getDefinition())
                goto CreateNewDecl;
            } else if (TagDecl *Def = PrevTagDecl->getDefinition()) {
    ...
    // things like templates still live in the tag namespace.
      ...
      // case here.
        ...
        // Tales NOTE: in PH files anon tag typedefs are turned into forward declarations
        if (!IsPHParsing) {
          unsigned Kind = 0;
      ...
      if (!IsPHParsing) // Keep the TypedefNameDecl
        Previous.clear();
    }
  }
  ...
  if (IsPHParsing) {
    assert(!Invalid);
    NamedDecl* PrevDef = (*Previous.begin())->getUnderlyingDecl();

    if (isa<TypedefNameDecl>(PrevDef))
      PrevDef = dyn_cast<TypedefNameDecl>(PrevDef);
    else
      PrevDef = cast<TagDecl>(PrevDef)->getDefinition();

    PrevDef->Whitelist();
    return PrevDef;  // FIXME? should be the underlying type decl for Typedefs
  }

CreateNewDecl:

  if (IsPHParsing) {
    Diag(NameLoc, diag::err_whitelisted_no_matching_decl_found)
          << Name;
    return 0;
  }
...
//   if (!IsPHParsing || Tag)  // During PH parsing we skip anonymous struct scopes
    PopDeclContext();
...
FieldDecl *Sema::HandleField(Scope *S, RecordDecl *Record,
  ...
  if (!PrevDecl && IsPHParsing) {
    Diag(Loc, diag::err_whitelisted_no_matching_fielddecl_found);
    return 0;
  }
...
void Sema::ActOnLateDecl(NamedDecl *ND) {
  ND->setLate(true);
  ...
  if (FunctionDecl *FD = dyn_cast<FunctionDecl>(ND)) {
    ...
    if (!ResultType->isDependentType() && !ResultType->isVoidType() &&
        !FD->isInvalidDecl() &&
        RequireCompleteType(FD->getLocation(), ResultType,
                            diag::err_func_decl_incomplete_result))
      FD->setInvalidDecl();
  }
