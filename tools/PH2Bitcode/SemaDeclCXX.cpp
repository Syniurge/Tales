...
bool Sema::MergeCXXFunctionDecl(FunctionDecl *New, FunctionDecl *Old,
  ...
  for (unsigned p = 0, NumParams = Old->getNumParams(); p < NumParams; ++p) {
    ...
    if (OldParamHasDfl && NewParamHasDfl && !IsPHParsing) {
...
Decl *Sema::ActOnStartNamespaceDef(Scope *NamespcScope,
  ...
  if (IsPHParsing) {
    if (!PrevNS) {
      Diag(Loc, diag::err_whitelisted_no_matching_decl_found) << II;
      return 0;
    }

    PrevNS = PrevNS->getOriginalNamespace();
    PrevNS->Whitelist();
    PushDeclContext(NamespcScope, PrevNS);
    return PrevNS;
  }
