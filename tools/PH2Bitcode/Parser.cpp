...
bool Parser::ParseTopLevelDecl(DeclGroupPtrTy &Result) {
  ...
  if (Tok.is(tok::eof)) {
    if (Actions.IsPHParsing)
      return true;

    // NOTE: Not sure if ugly? Should it be in Sema?
    Sema::LateDeclContainer& LateDecls = Actions.LateDecls;
    if (!LateDecls.empty()) {
      NamedDecl *ND = LateDecls.back();
      LateDecls.pop_back();

      Actions.ActOnLateDecl(ND);
      Result = Actions.ConvertDeclToDeclGroup(ND);
      return false;
    }
