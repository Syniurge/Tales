...
public:
  ...
  /// For Tales
  /// IsPHParsing - when parsing PH we only whitelist existing declarations
  /// in the AST built in the previous pass from the real header.
  bool IsPHParsing;
  StringRef PHFileName;

  bool IsPH2BC() const { return !PHFileName.empty(); }

  // Decls to be completed and emitted at the end of the translation unit
  // once all types are parsed.
  typedef std::vector<NamedDecl *> LateDeclContainer;
  LateDeclContainer LateDecls;

  void ActOnLateDecl(NamedDecl *D);
