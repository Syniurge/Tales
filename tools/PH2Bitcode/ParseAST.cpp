...
void clang::ParseAST(Sema &S, bool PrintStats, bool SkipFunctionBodies) {
  ...
  // Now for the whitelisting pass

  if (!S.PHFileName.empty()) {
    const FileEntry *File = S.getSourceManager().getFileManager().getFile(S.PHFileName);
    if (!File) {
      S.getDiagnostics().Report(diag::err_fe_error_reading) << S.PHFileName;
      assert(File);
      return;
    }
    S.getSourceManager().createMainFileID(File, SrcMgr::C_User);

    S.getPreprocessor().EnterMainSourceFile(); // breaks anything?
    P.ConsumeToken();

    S.IsPHParsing = true;
    llvm::errs() << "===== WHITELISTING PASS =====\n";
    Parser::DeclGroupPtrTy ADecl;
    while (!P.ParseTopLevelDecl(ADecl)) {}
    S.IsPHParsing = false;
  }
